...
class TwoInputCustomDialog(QDialog):
    ...
    def get_inputs(self):
        ...
        return (
            self.input1_field.text(),
            self.input2_field.text(),
...
"""
        ...
        self.editor.append(
            "Welcome to <b>EcoG's Codec Converter!</b><br>"
    ...
    def file_converter(
        self, from_codec: str, to_codec: str, convert_dir: str, del_files=False
    ):
    ...
    def show_two_input_dialog(self):
        ...
        if from_to_input.exec():
            ...
            self.valid_elements_lower = {item.lower() for item in ffmpeg_formats}
            self.c_from, self.c_to, self.del_files = from_to_input.get_inputs()
            if (
                self.c_from.lower() in self.valid_elements_lower
                and self.c_to.lower() in self.valid_elements_lower
            ):
                if self.del_files:
                    no = ""
                else:
                ...
                if selected_directory:
                    self.editor.append(
                        f"You selected the directory: {selected_directory}"
                        f"\nNow hit [ Go ] to start the convertion!"
                    )
                else:
                ...
                if self.c_from.lower() not in ffmpeg_formats:
                    QMessageBox.critical(
                        self, "Oops", f"{self.c_from} is not a valid format"
                    )
                else:
                    QMessageBox.critical(
                        self, "Oops", f"{self.c_to} is not a valid format"
                    )
    ...
    def go(self, c_from, c_to, del_files):
        ...
        msg_box.setInformativeText("<h2>Do you want to proceed with conversion?</h2>")
        ...
        if ret == QMessageBox.Apply:
            ...
            self.worker = LocalMetaWorker(
                c_from, c_to, self.selected_directory, del_files, self.file_converter
            )
...
class LocalMetaWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.callable(
                self.c_from, self.c_to, self.selected_directory, self.del_files
            )
...
class CodecsWizardPage(QWizardPage):
    def __init__(self, parent=None):
        ...
        self.registerField("from*", self.from_line_edit)  # Register 'from' field
    ...
    def update_status_labels(self):
        ...
        if is_from_valid and is_to_valid:
            ...
            self.status_label.setText("Status: **One or both formats are invalid** ❌")
    ...
    def isComplete(self):
        ...
        return self.is_valid_format(from_text) and self.is_valid_format(to_text)
...
class ConvertWizard(QWizard):
    ...
    def dir_page(self):
        ...
        def select_directory():
            ...
            label.setText(
                f"You selected: {selected_directory}\nNow hit [ Finish ] "
