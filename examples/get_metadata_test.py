...
def scan_library():
    ...
    try:
        result = subprocess.run(
            ["openfile.exe"], capture_output=True, text=True, timeout=60
        )
...
def get_basic_metadata(file_path):
    ...
    if audio_file is None:
        print(f"Could not read audio file: {file_path}. Make sure the file exists.")
        return None
    ...
    try:
        ...
        print(f"Error reading artist metadata from {file_path}: {str(e)}")
    ...
    try:
        ...
        print(f"Error reading album artist metadata from {file_path}: {str(e)}")
    ...
    try:
        ...
        print(f"Error reading title metadata from {file_path}: {str(e)}")
    ...
    try:
        ...
        print(f"Error reading album metadata from {file_path}: {str(e)}")
