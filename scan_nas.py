...
def get_audio_metadata(file_path):
    ...
    try:
        ...
        try:
            ...
            print(f"Error reading artist metadata from {file_path}: {e}")
        ...
        try:
            ...
            print(f"Error reading album artist metadata from {file_path}: {e}")
            # fallback to artist
            if metadata["artist"]:
                metadata["album_artist"] = metadata["artist"]
        ...
        try:
            ...
            print(f"Error reading title metadata from {file_path}: {e}")
        ...
        try:
            ...
            print(f"Error reading album metadata from {file_path}: {e}")
        ...
        try:
            ...
            print(f"Error reading year metadata from {file_path}: {e}")
        ...
        try:
            codec = (
                audio_file.mime[0]
                if hasattr(audio_file, "mime") and audio_file.mime
                else audio_file.__class__.__name__
            )
        ...
        try:
            ...
            print(f"Error detecting transition duration from {file_path}: {e}")
        ...
        if results:
            ...
            try:
                ...
                for result in results:
                    ...
                    added_covers += 1
                    print(f"Added cover from album: {album}")
                else:
                    print(f"Could not get album art from album: {album}")
                    errors += 1
                conn.commit()
                conn.close()
            except sqlite3.Error as e:
    ...
    try:
        ...
        for file_path in playlist_files:
            try:
                ...
                print(f"Database error adding playlist {file_path}: {e}")
                errors += 1
            except Exception as e:
                print(f"Unexpected error adding playlist {file_path}: {e}")
                errors += 1

        conn.commit()
        conn.close()

        print(f"Successfully added {added_playlists} playlists to database")
        if errors > 0:
            print(f"Encountered {errors} errors while adding playlists")
