...
<head>
    ...
    <style>
        ...
        .playlist-display { margin: 20px 0; }
        .playlist-songs { max-height: 300px; overflow-y: auto; border: 1px solid #ddd; }
        .playlist-song { padding: 10px; border-bottom: 1px solid #eee; cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
        .playlist-song:hover { background-color: #f0f0f0; }
        .playlist-song.current { background-color: #e3f2fd; font-weight: bold; }
        .playlist-song.played { opacity: 0.6; }
        .song-info { flex: 1; }
        .song-number { margin-right: 10px; color: #666; }
        .toggle-playlist { margin: 10px 0; padding: 5px 10px; background-color: #f8f9fa; border: 1px solid #ddd; cursor: pointer; }
    </style>
</head>
<body>
    <h1>Media Player</h1>
    ...
    <script>
        ...
        function setupNextSong() {
            ...
            currentAudioElement.addEventListener('timeupdate', function() {
                ...
                if (timeLeft <= crossfadeTime && currentIndex + 1 < playlist.length && !hasPreloaded) {
                    preloadNextSong();
                    hasPreloaded = true;
                }
            });
