...
class ScanWorker(QObject):
    ...
    def run(self):
        try:
            ...
            if not os.path.isdir(self.directory):
                self.error.emit(f"Path is not a directory: {self.directory}")
                self.finished.emit([], [], 0)
                return
            ...
            for root, dirs, files in os.walk(self.directory):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        self.warning.emit(f"Error processing file {file}: {e}")
            try:
            ...
            self.error.emit(f"Error scanning directory {self.directory}: {e}")
    ...
    def get_audio_metadata(self, file_path):
        ...
        try:
            ...
            try:
                ...
                self.warning.emit(f"Error reading album art from {file_path}: {str(e)}")
    ...
    def add_songs_to_database(self, audio_files):
        ...
        try:
            self.status.emit(f"Adding {len(audio_files)} audio files to database")
            ...
            for file_path in audio_files:
                try:
                    ...
                    if cursor.fetchone():
                        self.status.emit(f"File already in database: {file_path}")
                        continue
            ...
            self.status.emit(
                f"Successfully added {added_songs} songs and {added_covers} covers to database and encountered {errors} errors"
            )
        ...
        try:
            ...
            for file_path in playlist_files:
                try:
                    ...
                    cursor.execute(
                        "SELECT id FROM Playlists WHERE path = ?", (file_path,)
                    )
                    if cursor.fetchone():
                        self.status.emit(f"Playlist already in database: {file_path}")
                        continue
